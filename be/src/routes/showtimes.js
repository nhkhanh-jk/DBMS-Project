...
// GET /api/showtimes/:id/seats — sơ đồ ghế cho một suất chiếu
router.get('/:id/seats', async (req, res, next) => {
  try {
    const result = await seatService.listSeatsByShowtime(req.params.id);
    res.json(result);
  } catch (error) {
    next(error);
  }
...
// PUT /api/showtimes/:id (ADMIN, NHANVIEN) - alias for PATCH
router.put('/:id', requireAuth, requireRole('ADMIN', 'NHANVIEN'), async (req, res, next) => {
  try {
    const result = await showtimeService.updateShowtime(req.params.id, req.body);
    res.json(result);
  } catch (error) {
