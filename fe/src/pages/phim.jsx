...
// Status mapping: backend status → frontend tab
const STATUS_MAP = {
  "now-showing": ["RELEASED", "ACTIVE"],
  "coming-soon": ["SCHEDULED", "COMING_SOON"],
  "sneak-show": ["SNEAK_SHOW"],
};
...
export default function PhimPage() {
  ...
  // Filter movies by current tab using status mapping
  const filteredMovies = movies.filter((movie) => {
    const allowedStatuses = STATUS_MAP[activeTab] || [];
    return allowedStatuses.includes(movie.status);
  });
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16">
              {filteredMovies.length > 0 ? (
                filteredMovies.map((movie, idx) => (
                  <div key={movie.id || idx} className="flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500">
                    {/* Poster Container */}
                    <Link to={`/phim/${movie.id}`} className="relative group mb-4 block">
                      <div className="border-[3px] border-black shadow-[5px_5px_0px_rgba(0,0,0,0.1)] overflow-hidden relative bg-black">
                        <Image
                          ...
                          src={movie.posterUrl || movie.image || FALLBACK_IMG}
                        />
                        ...
                        <RankingMedal rank={idx < 3 ? idx + 1 : 0} />
                      </div>
                    </Link>
                    ...
                    <div className="flex-grow">
                      ...
                      <div className="mt-2 text-[14px] leading-[1.6] text-[#333]">
                        <p><span className="font-bold">{t("genre")}:</span> {translateGenre(Array.isArray(movie.genres) ? movie.genres.join(", ") : (movie.genres || ""), i18n.language)}</p>
                        <p><span className="font-bold">{t("duration") || "Duration"}:</span> {translateDuration(`${movie.durationMin || "?"} phút`, i18n.language)}</p>
                        <p><span className="font-bold">{t("release-date")}:</span> {movie.releaseDate ? new Date(movie.releaseDate).toLocaleDateString("vi-VN") : "?"}</p>
                      </div>
                    </div>
