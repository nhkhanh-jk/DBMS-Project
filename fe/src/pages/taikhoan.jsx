...
export default function TaiKhoanPage() {
  ...
  const handleUpdateProfile = async () => {
    ...
    setUpdateMsg("");
    try {
      await authApi.updateProfile({ fullName, phoneNumber });
      await refreshProfile();
      setUpdateMsg("✅ Cập nhật thành công!");
    } catch (err) {
      setUpdateMsg("❌ " + (err.response?.data?.error || "Cập nhật thất bại."));
    } finally {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      setPwMsg("❌ " + (err.response?.data?.error || "Đổi mật khẩu thất bại."));
    } finally {
      setPwLoading(false);
    }
  };
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row gap-6 items-start border-b border-gray-200 pb-8">
              ...
              <div className="flex-grow pt-2">
                <p className="text-lg font-bold text-[#333]">{t("hello")} {user?.fullName || user?.username || "..."}</p>
                <p className="text-sm text-[#666] mt-1 italic">{t("manage-account-desc")}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 border border-gray-300 rounded-lg overflow-hidden shadow-sm">
                <div className="p-4 border-b md:border-b-0 md:border-r border-gray-200 bg-white">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-blue-500 text-xl font-bold">★ {user?.membershipLevel || t("tnc-member")}</span>
                    </div>
                </div>
                <div className="p-4 border-b md:border-b-0 md:border-r border-gray-200 bg-white">
                    <p className="text-[11px] text-[#666] font-bold uppercase mb-2">{t("reward-points") || "Điểm tích lũy"}</p>
                    <div className="flex flex-col gap-2">
                        <span className="text-xl font-black">{user?.rewardPoints || 0}</span>
                    </div>
                </div>
                <div className="p-4 border-b md:border-b-0 md:border-r border-gray-200 bg-white">
                    <p className="text-[11px] text-[#666] font-bold uppercase mb-2">Email</p>
                    <div className="flex flex-col gap-2">
                        <span className="text-sm font-bold truncate">{user?.email || "-"}</span>
                    </div>
                </div>
                <div className="p-4 bg-white">
                    <p className="text-[11px] text-[#666] font-bold uppercase mb-2">{t("phone-number")}</p>
                    <div className="flex flex-col gap-2">
                        <span className="text-xl font-black">{user?.phoneNumber || "-"}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white border border-gray-200 p-6 rounded-sm shadow-sm">
                ...
                <div className="space-y-4 text-sm">
                    ...
                    <div className="grid grid-cols-[120px_1fr] border-b border-gray-100 pb-2">
                        ...
                        <span className="text-[#333]">{user?.phoneNumber || "-"}</span>
                    </div>
                </div>
            </div>
          </div>
      ...
      case "DETAILS":
        return (
          <div className="bg-white p-8 border border-gray-300 rounded-sm shadow-sm space-y-6">
             ...
             {updateMsg && <p className={`text-sm font-bold ${updateMsg.startsWith("✅") ? "text-green-600" : "text-red-500"}`}>{updateMsg}</p>}
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                <Input label={t("full-name")} value={fullName} onValueChange={setFullName} variant="bordered" radius="none" />
                <Input label={t("phone-number")} value={phoneNumber} onValueChange={setPhoneNumber} variant="bordered" radius="none" />
                <Input label="Email" value={user?.email || ""} variant="bordered" radius="none" isDisabled />
             </div>
             <Button isLoading={updateLoading} onClick={handleUpdateProfile} className="bg-[#b11116] text-white font-bold w-full md:w-40" radius="none">{t("save-info")}</Button>
          </div>
        );
      case "PASSWORD":
        return (
          <div className="bg-white p-8 border border-gray-300 rounded-sm shadow-sm space-y-6">
             ...
             {pwMsg && <p className={`text-sm font-bold ${pwMsg.startsWith("✅") ? "text-green-600" : "text-red-500"}`}>{pwMsg}</p>}
             <div className="max-w-md space-y-4 pt-4">
                <Input label={t("current-password")} type="password" value={currentPassword} onValueChange={setCurrentPassword} variant="bordered" radius="none" isRequired />
                <Input label={t("new-password")} type="password" value={newPassword} onValueChange={setNewPassword} variant="bordered" radius="none" isRequired />
                <Input label={t("confirm-password-label")} type="password" value={confirmNewPassword} onValueChange={setConfirmNewPassword} variant="bordered" radius="none" isRequired />
             </div>
             <Button isLoading={pwLoading} onClick={handleChangePassword} className="bg-[#b11116] text-white font-bold w-full md:w-40" radius="none">{t("update")}</Button>
          </div>
        );
       case "HISTORY":
        return (
